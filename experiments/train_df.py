        ...
        for epoch in range(epochs):
            for batch_np in iter_batches(
                ...
                if (global_step % log_every) == 0:
                    ...
                    f.flush()
                    print(
                        f"step={global_step} epoch={epoch} nll={float(loss):.6f} "
                        f"score| p50={p50:.3g} p99={p99:.3g} max={smax:.3g}",
                        flush=True,
                    )
                    pbar.set_postfix(nll=float(loss), p99=p99, smax=smax, epoch=epoch)
...
def main() -> int:
    ...
    run_df_training(cfg, args.data, args.run_dir, resume=args.resume)
    return 0
