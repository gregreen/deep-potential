...
def get_phi_loss_gradients(phi, frameshift, params, q, p,
    ...
    Inputs:
        ...
        frameshift (callable): Frameshift container for indicating which
            ...
            to u and du/dt defined by the frameshift. If set to None,
    ...
    with tf.GradientTape(watch_accessed_variables=False) as g:
        ...
        if frameshift is None:
            ...
            u, du_dt = frameshift(q, p)

            # TODO: No weighing
            
            null_hyp = tf.reduce_sum((p - u) * df_dq - (dphi_dq + du_dt) * df_dp, axis=1)
...
class FrameShift(tf.Module):
    """
    A 7-parameter model to represent the rotating frame in which f is stationary.
    The rotation axis is (0, 0, \Omega) and passes through a point at position
    (r_c, theta_c, phi_c). LSR moves with speed (u_LSRx, u_LSRy, u_LSRz).
    Typically, theta_c and phi_c are fixed so there are 5 free parameters.
    """

    def __init__(self, n_dim=3, omega0=0., r_c0=0., theta_c0=0.,
                phi_c0=0., u_LSRx0=-0.05, u_LSRy0=0.08, u_LSRz0=0.05, name='FrameShift'):
        """
        ...
        # Set the variables. theta_c0 and phi_c0 are fixed
        # spiral rotation 18-24, bar 
        self._omega = tf.Variable(omega0, trainable=False, name='omega', dtype=tf.float32)
        self._r_c = tf.Variable(r_c0, trainable=False, name='r_c', dtype=tf.float32)
        self._theta_c = tf.Variable(theta_c0, trainable=False, name='theta_c', dtype=tf.float32)
        self._phi_c = tf.Variable(phi_c0, trainable=False, name='phi_c', dtype=tf.float32)
    ...
    def __call__(self, q, p):
        # Returns u and du/dt, the flows defined by the frame shift
        n = q.shape[0]

        # set LSR velocity
        _u_LSR = tf.stack([self._u_LSRx, self._u_LSRy, self._u_LSRz], axis=0)
        u = tf.tile(tf.reshape(_u_LSR, [1, self._n_dim]), tf.constant([n, 1]))

        du_dt = tf.constant(0, shape=(n, self._n_dim), dtype=tf.float32)
        return u, du_dt
    ...
    def debug(self):
        print(f'name={self.name}\n  n_dim={self._n_dim}\n  omega={self._omega.numpy():.4f}\n\
  r_c=(r={self._r_c.numpy():.4f}, theta={self._theta_c.numpy():.4f}, phi={self._phi_c.numpy():.4f})\n\
...
def train_potential(
    ...
    if frameshift_model is not None:
        ...
        n_variables_frameshift = sum([int(tf.size(param)) for param in frameshift_param])
    ...
    if checkpoint_every is not None:
        if frameshift_model is None:
            ...
            checkpoint = tf.train.Checkpoint(opt=opt, phi=phi_model, frameshift=frameshift_model, step=step)
